...
Internet Gateway and Virtual Private Gateway have a $0 gateway-resource baseline in this model. VPN connections are separate catalog components with their own planning baseline; dedicated links and other transfer charges are excluded. This is not a zero-cost estimate for a working private network.

In legacy diagrams without network attachments, both gateways can start a graph path when they have no incoming edge. Internet Gateway is public ingress for the existing exposure/WAF checks; Virtual Private Gateway is private ingress and does not by itself trigger those checks. Both are treated as managed regional redundancy, not global services. Default placement has no individual availability zone; component and regional failures still interrupt their paths. Explicit subnet internet/VPN routes now affect reachability. Arbitrary CIDR routing, real tunnel health, failover capacity, and live provisioning remain outside this abstraction. See [network modeling](NETWORKING.md).

The Internet Gateway configuration is empty; attached networks use subnet routes, while legacy graphs use connections. Virtual Private Gateway has a private `asn`, default `64512`, accepting `64512–65534` or `4200000000–4294967294`. See the provider's [Internet Gateway model](https://docs.aws.amazon.com/vpc/latest/userguide/VPC_Internet_Gateway.html) and [Virtual Private Gateway setup](https://docs.aws.amazon.com/vpn/latest/s2svpn/SetUpVPNConnections.html). These settings describe a design; editing an ASN here does not modify a deployed gateway.
